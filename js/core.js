...
async function logDeletion(kind, info){
  ...
  try{
    ...
    if(typeof toast==='function') toast('Activity logged ✓');
  }
  catch(e){
    const reason = e && e.message ? e.message : String(e);
    console.error('Audit log write failed:', reason);
    if(typeof toast==='function') toast('Audit log failed — '+reason, true);
  }
}
...
/* ═══════════════════════════════════════
   APP STATE
═══════════════════════════════════════ */
const DEF = { boardingRate:55, daycareRate:35, threshold:3, surchargePct:50, bizName:'Shvaan Pet Care', bizPhone:'', bizEmail:'', bizAddr:'', capacity:12, theme:'terracotta' };
